...
Python, like R, has a host of extra packages to help with data import, wrangling, plotting & building various kinds of models. The first step is to load the packages we will need. I use the [Anaconda python distribution](https://www.anaconda.com/products/distribution) and packages that are not installed by default can be installed with the [`conda`](https://conda.io/projects/conda/en/latest/user-guide/getting-started.html) tool. In this post we use the `pymc` package for Bayesian modeling. The [installation notes]() for `pymc` recommend installing it into its own python environment so this is what I did! To run the code in RStudio I selected the relevant python environment in the Tools -> Global Options -> Python section and installed the other packages required into that environment.

![Python environment](python_pymc_env_rstudio.png)
...
::: {.column-margin}
As a quick aside `pymc3` has very recently been updated and renamed [`pymc`](https://www.pymc.io/welcome.html). Demonstrating with `pymc` would be great but I couldn't get [VS Code](https://code.visualstudio.com/) (which I used to write this post in 2022), `bambi` (see below) and `pymc` to play well together so we'll stick to `pymc3`. The niggles will probably be worked out soon(ish) and `pymc3` code can be used under `pymc` anyway.
:::
...
The `arviz` [@kumarArviZUnifiedLibrary2019a] package provides tools for summarising & plotting data from MCMC chains & posterior distributions.
