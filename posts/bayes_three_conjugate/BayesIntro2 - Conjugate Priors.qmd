...
The Binomial distribution is a discrete distribution because the number of trials ($n$) and the number of successes ($s$) can only be integers. In the current analysis the binomial distribution is useful in the *likelihood* function because in any single trial the number of successes and failures will be integers. 
...
In the analysis of the student marks we can use a Beta prior and a Binomial likelihood. If the prior and the likelihood share some mathematical form then the resulting posterior will be conjugate to the prior. The 'same mathematical form' parts of the Binomial & Beta distributions are shown in the figure below.
...
In the Beta distribution $a$ and $b$ should both be positive and together they describe the shape of a particular Beta distribution. The first parameter, $a$ can be thought of as the mode of the distribution i.e. where most of our probability will pile up. In this sense $a$ can be considered the 'number of successes' we think there should be. If we think there would be a lot of successes then the beta distribution will pile more probability closer to 1.

Here are several Beta distributions. Note the use of `dbeta` - we discussed the `d...()`, `p...()`, `q...()` & `r...()` distribution functions in R last time.
...
As $a$ increases the mode of the distribution moves right indicating more belief in an increased probability of success ($\theta$ - the x-axis). Conversely if $a$ gets smaller then the mode moves left.
...
Since we're multiplying the same numbers ($\theta$ and $1-\theta$) raised to different powers we can use the Laws of Exponents and all we have to do is raise $\theta$ and $1-\theta$ to the sum of powers.
...
Getting back to our question of the proportion of students who get first class marks we'll begin with flat prior i.e. we initially believe that any proportion from 0 to 100% is realistic. We can generate this as a Beta distribution with $a$ = 1 and $b$ = 1.
...
As you can see this is a uniform distribution - the probability is exactly the same for every value proportion ($\theta$) on the x-axis.

Now let's calculate our posterior distribution. We just have to add the number of successes to the $a$ parameter of our Beta prior which is 1; then calculate n minus successes and add that to the $b$ parameter of our Beta prior which is also 1. We'll then plot our prior and our posterior together.
...
Sweet! We've updated our view of the world based on some data. Where we thought every proportion was equally plausible before the data suggests some support for about 1-20%. And all we had to do was add some numbers together!
...
b_post
```
So $a$ is 5 and $b$ is 58 in our posterior.
...
The blue part is where we believe the true proportion, $theta$ of first class marks lies with 95% probability.

### An informed prior

We'll now examine the student data using a more informed prior. In the [previous post](https://iaingallagher.github.io/posts/2_bayes_grid_approx/Bayes_part_2_grid_estimation.html) my initial belief was that around 10% of students would achieve a first class mark and this is somewhat supported by the analysis above. To reflect my belief in 10% I'll create a semi-informative prior Beta distribution around 10%. 

One way to approach the parameters $a$ and $b$ required for this Beta prior is to consider that the **mean** proportion of successes will be $m = a/(a+b)$ and the sample size ($n$) will be $n = a+b$. From this we can see that $a = m \times n$ and $b = (1-m)n$. 

The value of $m$ is our guess for the mean value of $\theta$ before we see data or based on our domain expertise - here that's 0.1 (i.e. 10%). The value of $n$ can be thought of as the amount of data we have previously seen which informs our prior guesstimate for $\theta$. In this case although we're guessing we'd feel somewhat happy basing our belief on 20 previous data points.
...
Although the estimate of $\theta$ starts off at 80% (prior in the top left plot) we can see that the data pulls the posterior distribution over to the lower end of the possible proportions. Once we have analysed all our data the prior and posterior are approaching the same estimate as before. This demonstrates that a lot of data will overwhelm a poor choice prior. However you may not always have a lot of data! The width of the prior also makes a difference. If we are very confident about a value then we might have a very narrow prior over that value. It then takes more data to shift that prior to other values.
